...
top50_prop <- 
  ...
  filter(school_id <= 50 ) %>% 
  mutate(
    application_total_prop = application_total_count/total_application_2023
  ) %>% 
...
We suspect that the size of the city in which the university is located will influence the number of application submitted to that university. Let's test this hypothesis. 
...
d2 <- 
  ds3 %>% 
  filter(school_id <= 50) %>% 
  select(school_id, school_abb, student_count, application_total_count, city_population, city ) %>% 
  mutate(
...
g2 <- 
  d2 %>% 
  ggplot(aes(x=city_population, y = student_count))+
  geom_point()
# Homework: isolate the largest university (student_count) visually and add the 
# ggrepel labels to them in the same way we did in graph 1. 
# it looks like the size of the city does not influence the size of the student body
g2
